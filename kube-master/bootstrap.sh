...
sudo kubeadm init --v=5 --cri-socket unix:///var/run/cri-dockerd.sock --kubernetes-version v$KUBE_VER --image-repository registry.aliyuncs.com/google_containers --pod-network-cidr=10.244.0.0/16 --apiserver-advertise-address 192.168.33.20 | tee /shared/kubeadm.log
